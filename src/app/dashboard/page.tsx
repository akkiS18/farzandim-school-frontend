...
function TenantDashboardContent() {
  ...
  return (
    <main className="min-h-screen bg-[#F7F7FA] text-[#1D1E26] flex font-sans overflow-hidden p-0">
      <div className="flex w-full h-[100dvh] bg-[#F7F7FA] overflow-hidden border-none shadow-none rounded-none">
        {/* Sidebar Navigation */}
        <Sidebar
        ...
        <section className="flex-1 h-full overflow-y-auto px-4 sm:px-8 py-6">
          <div className="max-w-7xl mx-auto space-y-6 pb-12">
            ...
            {activeMenu === "overview" && (
              <OverviewSection
                ...
                classes={classes}
                userInfo={userInfo}
                setActiveMenu={setActiveMenu as any}
              />
            )}
          ...
          {activeMenu === "telegram" && (
            <div className="space-y-6 max-w-2xl mx-auto font-sans text-[#1D1E26] select-none">
              <div className="bg-white border border-slate-200 rounded-none p-6 shadow-xs space-y-6">
                <div className="border-b border-slate-100 pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <span className={`px-2.5 py-1 text-[10px] font-mono font-extrabold shrink-0 border self-start sm:self-center ${
                    telegramConfig?.has_token
                      ? "bg-slate-100 text-[#1D1E26] border-slate-200"
                      : "bg-slate-50 text-slate-400 border-slate-200"
                  }`}>
                    {telegramConfig?.has_token ? "Bot ulangan" : "Bot ulanmagan"}
                  </span>
                </div>
                ...
                {telegramLoading ? (
                  ...
                  <form onSubmit={handleSaveTelegramConfig} className="space-y-4">
                    ...
                    {telegramSuccess && (
                      <div className="p-3.5 bg-slate-100 border border-slate-200 text-[#1D1E26] text-xs font-extrabold rounded-none">
                        {telegramSuccess}
                      </div>
                    )}
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        className="w-full text-xs border border-slate-200 rounded-none px-3.5 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#1D1E26] bg-slate-50 font-mono font-bold text-slate-800 transition"
                    ...
                    {telegramConfig?.has_token && (
                      <div className="bg-slate-50 border border-slate-200 rounded-none p-4 space-y-2">
                        ...
                        <div className="text-xs space-y-1.5 text-slate-700 font-semibold">
                          <p>
                            Bot nomi: <a href={`https://t.me/${telegramConfig.bot_username}`} target="_blank" rel="noopener noreferrer" className="text-[#1D1E26] font-bold hover:underline">@{telegramConfig.bot_username}</a>
                          </p>
                          <p>
                            Ota-onalar uchun taklif havolasi: <code className="bg-white px-2 py-0.5 rounded-none text-[10px] select-all font-mono text-[#1D1E26] border border-slate-200">https://t.me/{telegramConfig.bot_username}?start=1</code>
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex justify-end pt-2 border-t border-slate-100">
                      <button
                        ...
                        className="px-5 py-2.5 bg-[#1D1E26] text-[#D4F562] hover:bg-slate-800 font-extrabold rounded-none text-xs transition flex items-center space-x-1.5 cursor-pointer disabled:opacity-50"
                      >
                        {telegramSaveLoading && <span className="w-3.5 h-3.5 border border-[#D4F562] border-t-transparent rounded-full animate-spin shrink-0"></span>}
